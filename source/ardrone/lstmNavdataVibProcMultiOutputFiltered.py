...
modelFilename = 'cache/lstm_navdatavib_model_multioutfilter_aug19_' + str(datetime.now().strftime('%y_%m_%d_%H_%M_%S')) 
...
def createModel(early_stop=True, checkpoint=True):
    ...
    if checkpoint:
        addOns.append(ModelCheckpoint('D:/Dataset/ARDrone/modelCheckpoint/model-' + featureName[outFeatureNum] + '-{epoch:03d}-{loss:03f}-{val_loss:03f}.h5', save_best_only=True, monitor='val_loss', mode='min'))
