    ...
    for nd in range(nData):
        ...
        if dataArr[nd+n_sequence-1]['loadNum'] >= 4 or dataArr[nd+n_sequence-1]['loadNum'] == 0:
            ...
            for ns in range(n_sequence):
                ...
                featureArrInput = np.array([
                    np.mean(dataArr[nd+ns][motKey]['rms']),
                    np.mean(dataArr[nd+ns][motKey]['kurtosis']),
                    np.mean(dataArr[nd+ns][motKey]['skewness']),
                    np.mean(dataArr[nd+ns][motKey]['crest-factor']),
                    np.mean(dataArr[nd+ns][motKey]['peak-to-peak'])
        ...
        # Normal motor
            ...
            for ns in range(n_sequence):
                ...
                # Add input feature array
                featureArrInput = np.array([
                    np.mean(dataArr[nd+ns]['mot1']['rms']),
                    np.mean(dataArr[nd+ns]['mot1']['kurtosis']),
                    np.mean(dataArr[nd+ns]['mot1']['skewness']),
                    np.mean(dataArr[nd+ns]['mot1']['crest-factor']),
                    np.mean(dataArr[nd+ns]['mot1']['peak-to-peak'])
                ]).reshape(1,n_feature[0])
...
### Dataset splitting (random sampling)
testDataCount = int(nTest * len(aggregateDataset))
testDataset = []
for _ in range(testDataCount):
    popData = aggregateDataset.pop(random.randrange(len(aggregateDataset)))
    testDataset.append(popData)

trainDataset = aggregateDataset
