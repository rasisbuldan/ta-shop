...
##### Real Data #####
dataPath = 'D:/Cloud/Google Drive/Tugas Akhir/data/dumps/seq_agg_load6'
...
featureDataSelectedArr = featureDataSelectedArr.loc[2:]
pca = PCA(n_components=1)
pca.fit(featureDataSelectedArr)
featureDataSelectedArr['HI'] = pca.transform(featureDataSelectedArr)
featureDataSelectedArrEWM = featureDataSelectedArr.ewm(alpha=0.02).mean()
...
pca.fit(featureDataSelectedArr)
featureDataSelectedArr['HI'] = pca.transform(featureDataSelectedArr)
featureDataSelectedArrEWM = featureDataSelectedArr.ewm(alpha=0.001).mean()
featureDataSelectedArr['HIs'] = featureDataSelectedArrEWM['HI']
...
p1, = plt.plot(list(range(len(realData))), realData['HIss'], 'r-')
p2, = plt.plot(list(range(offset[0],len(predictedData)+offset[0])), predictedData['HIss'], 'k-')
plt.grid(True)
plt.legend((p1,p2),('Real Data', 'Predicted Data'), fontsize=18)
plt.show()